...
export default function PlacePage({ isLoggedIn, userRole, currentUser }) {
  ...
  useEffect(() => {
    fetch(`${BASE}/api/place/${id}`, { credentials: 'include' })
      .then(async res => {
  ...
  useEffect(() => {
    if (categoryId && !categoryName) {
      fetch(`${BASE}/api/category/${categoryId}`, { credentials: 'include' })
  ...
  const handleAddToTrack = async () => {
    ...
    try {
      const response = await fetch(`${BASE}/api/user/favorites`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(t || 'Failed to add to track');
      }
      setIsFavorited(true);
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/api/place/${id}/review`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      savedReview.author = savedReview.author || currentUser?.full_name || currentUser?.username || 'Anonymous';
  ...
  const handleDeleteReview = reviewId => {
    ...
    fetch(`${BASE}/api/review/${reviewId}`, { method: 'DELETE', credentials: 'include' })
      .then(res => {
        if (res.ok) {
          setPlace(prev => {
  ...
  const renderStars = value =>
    [...Array(5)].map((_, i) => (
      <FaStar key={i} color={i < value ? '#ed0000' : '#ccc'} size={16} style={{ marginRight: 2 }} />
    ));
            ...
            <div className="rating-input">
              ...
              {[1,2,3,4,5].map(star => (
                <span
                  ...
                  style={{ color: star <= rating ? '#ed0000' : '#ccc', cursor: 'pointer', fontSize: '1.5rem' }}
                >
                  ★
                </span>
              ))}
          ...
          {place.reviews?.map(review => (
            <div key={review.id} className="review">
              {review.user_id ? (
                ...
                <strong>{review.full_name || review.author || 'Anonymous'}</strong>
              )}
